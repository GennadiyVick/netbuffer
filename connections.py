...
class ConnectionsDialog(QtWidgets.QDialog):
    def __init__(self, connections, parent = None):
        ...
        self.model.setHorizontalHeaderLabels(['Название','IP','Порт'])
    ...
    def getConnections(self):
        ...
        for i in range(self.model.rowCount()):
            result.append({'name': self.model.item(i,0).text(),'ip': self.model.item(i,1).text(),'port': int(self.model.item(i,2).text())})
        return result
